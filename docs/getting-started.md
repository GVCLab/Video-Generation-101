...
以文生视频为例说明训练、条件编码、生成和解码的基本过程。

**前置知识：** 无需机器学习先修知识。

**使用步骤：** 区分模型训练与生成 → 理解输入条件和生成结果的关系 → 按实际任务进入模型、数据或评测专题。

本章用潜空间扩散模型说明视频生成的基本流程。自回归模型按顺序预测内容，掩码模型恢复被遮盖的内容，具体训练目标和采样过程见对应章节。

> 扩散式视频生成模型先从大量视频中学习“什么东西长什么样、通常怎样运动”，再根据你的文字或图片，从一团随机噪声中逐步做出一段视频。
...
> 图示以扩散式生成器为例。训练目标是学习视频与条件的统计关系；是否记忆或复制训练样本需要单独检查。
...
模型通过参数拟合训练数据的统计规律，并不要求显式求解物理方程。过拟合、重复数据或特殊提示仍可能导致训练样本的记忆和近似复现，因此不能仅凭生成过程排除数据泄漏。
...
训练完成后，扩散式文生视频系统通常按以下八步进行推理。具体实现可能合并部分步骤，其他生成路线的采样过程有所不同。
...
不建议按文件顺序从头读到尾。先建立全流程，再根据自己的问题选择路线。本页保留四条最常见的快捷入口；需要更细的读者画像、跳读规则、建议产出和跨路线迁移时，请使用[分读者阅读指引](reader-guides.md)。
