...
介绍权重、激活与缓存量化，以及校准、精度分配和低比特算子。

**前置知识：** 数值精度、视频扩散。

**使用步骤：** 定义量化对象、粒度与精度 → 使用覆盖时间步和运动分布的数据校准 → 同时检查数值误差、视频质量和实际时延。
...
本章纳入直接压缩视频扩散模型去噪网络、注意力、特征缓存或因果视频模型 KV 缓存的研究。图像扩散量化仅在其为时间步感知校准、异常值处理或底层算子协同设计提供方法基础时加以讨论 [[1]](#ref-1) [[9]](#ref-9)。

视频 tokenizer 的离散码本量化、训练数据压缩、仅调整 sampler 或 solver 以减少采样步数，以及没有低比特算子实现而只报告 FLOPs 的工作，不属于本章的主要范围。

现有证据主要来自各方法论文自身的实验，目前尚无统一的视频扩散量化 benchmark。因此，本章侧重比较研究问题与方法设计，不对不同论文给出脱离实验条件的速度排名。
