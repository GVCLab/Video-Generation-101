...
介绍候选采样、搜索、验证器与自适应预算如何影响生成质量。

**前置知识：** 视频生成、评测方法。

**使用步骤：** 建立完整的请求级成本记录 → 设置多个预算点与独立评价器 → 绘制质量成本曲线并分析饱和和退化。
...
**基本定义：Test-Time Scaling（TTS）不是单纯“多跑几步”，而是在每个请求到来后，把可调的推理预算分配给候选生成、轨迹搜索、验证、修复或临时适配，并考察最终质量是否随总预算稳定提升。**
...
智能体如何组织制作计划、工具调用、记忆与局部返工，见 [Agentic Video Generation](../agentic-video-generation.md)；本章负责判断额外预算是否带来可验证的质量提升。

<a id="1-test-time-scaling"></a>
