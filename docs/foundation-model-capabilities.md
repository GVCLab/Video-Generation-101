...
按视觉、时间、语义、控制和行动等维度描述模型能力，并区分能力、训练方法与产品功能。

**前置知识：** 视频生成基础。

**使用步骤：** 确定需要测量的能力 → 选择对应任务和指标 → 区分基础模型、后训练模型与完整系统的结果。


基本概念：**物理一致性是模型输出和行为层的能力；后训练是获得、增强或重塑能力的机制。** 如果要描述基础模型是否“容易后训练”，更准确的能力名称是**可适配性 / 可对齐性**，而不是“后训练能力”。Foundation model 的核心判据本来就是广泛预训练后可适配多类下游任务，而非是否采用某一种训练算法 [[1]](#ref-1)。
...
| **任务 / 接口** | 能力通过什么输入输出规格被调用？ | T2V、I2V、V2V、视频编辑、动作条件预测 | 把任务名直接当成内部能力 |
...
<a id="capability-cross-table-index"></a>

### 1.1 跨表格索引

能力地图是 C1–C9 的唯一释义入口，其他页面只引用编号，不重复定义。可以沿下面五张主表完成“能力 → 任务 → 获得方式 → 证据 → 应用/系统”的追踪：
...
| 结果应归因给 checkpoint 还是系统？ | [系统归因矩阵](foundation-models.md#system-attribution-matrix) | 能力声明必须落到可核验的模型或系统单位 |
...
|---|---|---|---|---|
| <a id="capability-c1"></a>**C1** | **生成质量与分布建模** | 单帧清晰度、纹理与结构、审美、伪影控制、样本多样性与分布覆盖 | 同一条件多随机种子；质量与覆盖分开报告；校准后人评或指标 | 条件正确、时间正确、物理正确 |
| <a id="capability-c2"></a>**C2** | **语义与组合遵循** | 对象、数量、属性绑定、空间语言关系、动作绑定、否定、事件顺序和复杂情节 | 把 prompt 拆为原子事实；检查绑定与顺序；记录拒绝和改写 | 关键词出现就代表关系或动作正确 |
| <a id="capability-c3"></a>**C3** | **时间动力学与长期状态** | 帧间连续、运动幅度与方向、身份持续、遮挡后恢复、不可逆状态、跨镜头记忆和长时漂移 | frame shuffle / freeze 压力测试；离场再现；按时长和镜头间隔报告退化 | 运动平滑就代表符合力学；可持续采样就代表长期可靠 |
...
| 编号 | 能力族 | 核心子能力 | 最低可接受证据 | 不能直接推出 |
|---|---|---|---|---|
| <a id="capability-c4"></a>**C4** | **空间、几何与多视角一致性** | 深度、遮挡、拓扑、相机运动、同刻多视角、可渲染 3D/4D 状态 | 固定时间变相机、固定相机变时间；新视角和重投影；遮挡与 loop closure | 文本中的“左/右”正确就代表 3D 正确 |
| <a id="capability-c5"></a>**C5** | **控制、编辑与个性化** | 文本可控、相机/轨迹/姿态/深度控制、局部与全局编辑、身份绑定、非目标内容保持 | 逐个加入、删除和冲突条件；同时测目标成功与非目标守恒 | UI 接受输入就代表模型使用了条件 |
| <a id="capability-c6"></a>**C6** | **多模态音视频协同** | 文本—视频、文本—音频与音频—视频语义一致，事件同步、口型同步、声源/说话人绑定、跨镜头声音状态 | 单模态干预；事件级时间偏移；身份与声源绑定；静音、缺失模态和长时测试 | “输出有声音”就代表原生联合生成 |
...
| 编号 | 能力族 | 核心子能力 | 最低可接受证据 | 不能直接推出 |
|---|---|---|---|---|
| <a id="capability-c7"></a>**C7** | **常识、物理与反事实一致性** | 对象永久性、接触与碰撞、重力、材料、连续介质、守恒、原因—结果和参数响应 | 逐规律诊断；固定初态只改变一个物理因素；检查差分方向和量级 | 单条视频看起来合理就代表学到物理定律 |
| <a id="capability-c8"></a>**C8** | **视觉理解与生成式推理** | 感知、分割、视觉操作、affordance、状态推进、规则归纳、问题求解和规划 | 未见任务；最终答案与中间状态双检；固定 pass@$k$、重试和计算预算 | 生成了过程帧就证明内部存在可解释推理算法 |
| <a id="capability-c9"></a>**C9** | **动作条件世界建模与决策** | 动作语义、分支未来、状态持续、不确定性、闭环响应、策略排序和真实任务收益 | 同初态动作分支；多步 rollout；闭环控制；独立环境中的 return / regret | 漂亮的未来视频就能支持正确行动 |
...
| 任务表面 | 通常调用的能力 | 仍需单独声明的规格 |
|---|---|---|
| T2V | [C1](#capability-c1) · [C2](#capability-c2) · [C3](#capability-c3) · [C7](#capability-c7)；复杂控制还需 [C5](#capability-c5) | 条件来源、seed、时长、拒绝与重试 |
| I2V | [C1](#capability-c1) · [C2](#capability-c2) · [C3](#capability-c3) · [C5](#capability-c5) · [C7](#capability-c7) | 锚点时刻、参考保持、运动自由度与多未来 |
| 视频编辑 / 个性化 | [C2](#capability-c2) · [C3](#capability-c3) · [C5](#capability-c5) | 哪些内容必须改变，哪些必须保持 |
| 多镜头故事 | [C1](#capability-c1) · [C2](#capability-c2) · [C3](#capability-c3) · [C4](#capability-c4) · [C5](#capability-c5) · [C7](#capability-c7) | 镜头计划、实体账本、不可逆事件和回滚 |
| 原生音视频 | [C1](#capability-c1) · [C2](#capability-c2) · [C3](#capability-c3) · [C6](#capability-c6) · [C7](#capability-c7) | 两个模态是否联合采样，还是后配音 |
| 多视角 / 4D | [C1](#capability-c1) · [C3](#capability-c3) · [C4](#capability-c4) | camera–time 网格、输出是像素还是可查询状态 |
| Video Reasoning | [C2](#capability-c2) · [C3](#capability-c3) · [C7](#capability-c7) · [C8](#capability-c8) | 答案、过程、预算和外部工具 |
| 动作条件预测 / 交互世界 | [C3](#capability-c3) · [C4](#capability-c4) · [C7](#capability-c7) · [C9](#capability-c9)；含显式规划或推理时再加 [C8](#capability-c8) | 动作空间、反馈频率、horizon、闭环环境 |
...
VideoDPO 用多维偏好构造视频生成的 DPO 训练 [[13]](#ref-13)；VideoAlign 同时比较训练期 Flow-DPO / Flow-RWR 和推理期 Flow-NRG，说明“更新权重”和“当次引导”必须分开 [[14]](#ref-14)；T2V-Turbo 把 reward 与 consistency distillation 组合，承担“偏好改善”和“少步生成”两份不同规格 [[15]](#ref-15)。完整方法、成本与 reward-hacking 审计见[视频后训练与对齐](generative-models/video-post-training-alignment.md)。
...
<a id="capability-attribution-matrix"></a>
...
| 一个视频基础模型系统怎样从数据走到服务？ | [视频基础模型系统及归因表](foundation-models.md#system-attribution-matrix) |
| 一个任务允许什么输入、必须保持什么？ | [任务—能力矩阵](taxonomy.md#task-capability-matrix) |
| 模型内部用什么表示、目标、骨干和部署机制？ | [生成模型路线](generative-models.md) |
| 后训练到底更新谁、花费什么、改善什么？ | [后训练—目标能力矩阵](generative-models/video-post-training-alignment.md#post-training-capability-matrix) |
| 一项能力声明需要什么指标和证据？ | [能力—评测矩阵](evaluation.md#capability-evaluation-matrix) |
