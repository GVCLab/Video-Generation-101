...
<a id="5-world-model"></a>

## 5. 世界模型评测

### 5.1 概念边界

文献中至少有三类系统被称为 World Model：观察历史后预测未来画面的**观察预测器**；建模 $p(o_{t+1},r_{t+1}\mid o_{\le t},a_{\le t})$ 的**动作条件环境模型**；以及以规划所需 latent transition、reward、value 或 policy 为主要评测对象的**决策型模型**。Sora 一类技术报告主要提供生成与视觉规律证据 [[28]](#ref-28)，PlaNet、MuZero 和 DreamerV3 则以规划或环境回报验证决策充分性 [[29]](#ref-29) [[30]](#ref-30) [[31]](#ref-31)。三者不能共享同一证据标准。

### 5.2 证据阶梯

以下 L0–L7 是本手册统一使用的**报告约定**，用于区分实验支持的结论，未作为社区公认标准发布。它们不是可相加的分数，也不意味着通过某一层就自动通过此前各层。稳健性、安全性与不确定性需要在相关层分别报告。

| 层级 | 评测对象 | 推荐实验 | 结论范围 |
|---|---|---|---|
| L0 | 单帧视觉质量 | 感知质量、人评、伪影检查 | 指定分布下的视觉保真 |
| L1 | 时间连续性与内容一致性 | 运动连续性、身份保持、跨帧误差 | 指定时长内的时序表现 |
| L2 | 语义与可见物理规律 | 指令遵循、对象关系、物理诊断 | 被测试事件中的规律拟合 |
| L3 | 动作条件响应 | 固定状态换动作、no-op 对照、状态转移误差 | 对明确动作空间的响应 |
| L4 | 反事实分支 | 同状态异动作、独立分支真值、条件分布校准 | 被测试干预下的预测有效性 |
| L5 | 持续交互与状态维护 | 闭环 rollout、回访、失败时间与时长曲线 | 特定环境中的交互稳定性 |
| L6 | 独立环境中的决策效用 | 策略排序、规划对照、回报与后悔值 | 独立模拟或测试环境中的决策收益 |
| L7 | 真实系统效用 | 真实部署、任务成功率、干预和故障统计 | 所测真实系统与任务中的收益 |
