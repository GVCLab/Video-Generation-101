...
介绍文本条件编码、视频生成路线、推理配置和语义评价。

**前置知识：** 视频生成入门。

**使用步骤：** 把文本要求拆成实体、动作和时间关系 → 固定模型、采样与输出参数 → 检查条件遵循、运动和生成多样性。
...
图 1 的顺序文本替代如下：用户意图先被拆成实体、关系、动作、时间、镜头与声音要求；prompt compiler 将它们变成可版本化的条件接口；文本编码器、可选控制器和生成器据此产生媒体 latent/token；解码或滚动生成输出视频与可选音频；最后分别检查 prompt binding、时间顺序、视觉连续、运动/物理、音视频同步、成本/延迟以及安全/来源。只有文本输入的是纯 T2V；加入来源像素的是混合生成；动作与新观测闭环才是 world model。
...
这条边界也解释了为什么 MoCoGAN、VideoGPT、Stable Video Diffusion 或 MAGI-1 不能不加说明地列为“纯 T2V 里程碑”：它们分别是通用视频生成祖先、token 生成祖先、主要 I2V 系统或文本条件 I2V/连续生成系统，对 T2V 很重要，但解决的条件接口不同。
...
| 2024 | CogVideoX [[12]](#ref-12)、Movie Gen [[13]](#ref-13)、HunyuanVideo [[14]](#ref-14) | 大规模 DiT/flow、文本编码与系统工程成为主线 | 不同发布内容使横向复现不对称 |
...
<a id="prompt-compiler-caption"></a>
...
| Step-Video-T2V [[16]](#ref-16) | 2025 | 30B T2V 系统、Video-VAE、flow 与 Video-DPO | 技术报告及作者发布内容 | “最多 204 帧”自动等于任意长视频稳定 |
...
| Ovi [[27]](#ref-27) | 2025 | 开放联合音视频双流路线 | 预印本、项目/发布内容按版本 | 等于封闭产品的全部音视频能力 |
