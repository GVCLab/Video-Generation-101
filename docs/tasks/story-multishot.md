...
| 图像故事 | story visualization、Make-A-Story | 逐图生成 + visual memory | 视频运动不足 |
| 分步 pipeline | LLM 分解故事、生成关键帧、I2V 动画 | 脚本到镜头计划 | 误差累积，风格漂移 |
...
单镜头 T2V 主要回答“这一段画面是什么”。多镜头生成要回答“前后镜头如何构成一个故事”。因此模型需要比普通视频生成更多结构：角色设定、场景地图、镜头计划、故事进度、视觉记忆和状态更新。许多最新方法不再一次性生成整片，而是先规划，再逐镜头生成，并用 memory 或 reference 保持一致。
...
- Make-A-Story：visual memory conditioned story generation。
- VideoGen-of-Thought：step-by-step multi-shot synthesis。
- Text-to-Multi-Shot Video Generation：全帧 attention 的多镜头生成。
- StoryMem、ShotStream：memory 与 streaming 趋势。
- MuSS、ConStoryBoard、AnimeShooter：数据与 benchmark。
