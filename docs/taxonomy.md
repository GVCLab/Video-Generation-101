...
1. **条件来源**：模型依据什么信息生成；
2. **与源内容的关系**：从零创作、受参考约束、变换已有内容，还是修复缺失内容；
...
![视频生成任务三轴地图。中央矩阵以条件来源为纵轴、与源内容的关系为横轴，放置无条件生成、文生视频、图生视频、视频预测、数字人、视频编辑、视频修复、帧插值、动作条件预测和交互世界等任务；上方独立的交互时域轴从一次性片段延伸到长期记忆和闭环控制。该图强调三轴可以组合，不是成熟度阶梯。](../assets/diagrams/video-generation-task-map.png)

**图 1：任务是坐标，不是等级。** 中央卡片给出常见主坐标，不表示任务只能接受一种输入；上方 `INTERACTION HORIZON` 是独立第三轴，因此没有用连线把每个任务永久绑定到某个时域。生成图经过两轮纠错：首稿纠正了插帧、预测和数字人的条件位置，次稿删除了会暗示固定关系的虚线。图像提示词、版本和 SHA-256 见[研究日志](../sources/research_20260830_task_application_taxonomy.md)。

~~~mermaid
flowchart LR
    accTitle: 视频生成任务的三个可组合坐标
    accDescr: 条件来源、与源内容的关系和交互时域共同形成任务合同；任务合同再确定必须保持的内容、允许改变的内容和相应评测，而不是先由模型名称决定任务。

    C["条件来源 C<br/>无 / 文字语义 / 图像视频音频 / 状态动作"]
    R["源内容关系 R<br/>从零创作 / 参考约束 / 变换编辑 / 修复补全"]
    H["交互时域 H<br/>open-loop clip / 长期记忆 / closed-loop"]
    T["任务合同 T<br/>输入 · 输出 · 不变量 · 干预 · 时域"]
    E["验收证据<br/>条件响应 · 保留误差 · 多样性 · deadline · 闭环效用"]
    M["模型实现<br/>AR / masked / diffusion / flow / GAN / hybrid"]
...
| 修复 / 补全 | 已知区域是硬约束，缺失区域需合理补齐 | inpainting、插帧 | mask 外泄漏、边界接缝、遮挡恢复错误 |

“参考约束”“变换”和“修复”之间可以重叠。VACE 把 reference、editing 与 mask 组织进统一条件接口，说明一个模型可以组合任务；这不意味着三个任务的保留合同和评测可以合并 [[5]](#ref-5)。
...
- 修复把已知像素视为证据，只在 mask 内补全；mask 外误差应单独报告。
- “统一模型都能做”只说明接口复用，不取消 outside-mask protection 指标。
...
flowchart TD
    accTitle: 从任务合同到证伪测试
    accDescr: 输入输出定义后，分别建立保持账本与变化账本，再对条件做反事实干预，并在多个随机种子、时间长度和系统负载下验收；未通过则回到任务或系统设计，而不是挑选最好样例。

    A["定义 I / O<br/>模态 · 单位 · 时间轴"] --> B["保持账本 K<br/>身份 · 几何 · mask 外 · 状态"]
    B --> C["变化账本 Δ<br/>对象 · 动作 · 风格 · 相机"]
    C --> D["反事实干预<br/>一次只改变一个条件"]
    D --> E["协议网格<br/>多 seed · 多时长 · 多难度 · 多负载"]
    E --> F{"全部硬门槛通过？"}
    F -- 否 --> G["记录失败、回滚<br/>修任务定义 / 数据 / 模型 / 系统"]
    F -- 是 --> H["报告分布与不确定性<br/>不是只展示最佳样例"]
    G --> A
~~~
...
| 局部编辑 / 修复 | 目标区变化 + 非目标区误差 | 移动 mask、细边界、快速运动、scene cut | 整段感知质量平均分 |
