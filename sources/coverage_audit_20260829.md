...
Counts are a repository snapshot after adding the causal-streaming, video post-training, native audio-video, fine-grained control, video degradation restoration, video-tokenizer and multi-view/4D chapters, with the mechanism, foundation, task-map, application, getting-started, interpolation, inpainting, text-to-video, image-to-video, video-to-video, video-prediction, action-conditioned prediction, interactive-world, digital-human, story/multi-shot, unconditional-video, open-model, video-reasoning, JEPA, diffusion/flow, physics, timeline, reading-route and bibliography rows refreshed on 2026-08-30. “Visual” counts Mermaid blocks plus embedded images; a visual can still fail the scientific-quality gate. The former mixed VAE/tokenizer page is now split: `variational-generation.md` owns ELBO, learned prior, posterior collapse and multi-future evidence, while `video-tokenizers.md` owns representation, quantization, causal/chunk boundaries, exact budgets, bitstream maturity and fixed-consumer replacement tests.

## A. Generation mechanisms and foundation models

| File | Lines | Refs | Visuals | Current depth | Main gap | Priority |
|---|---:|---:|---:|---:|---|---|
| `docs/generative-models.md` | 456 | 32 | 4 | 4 | Five-axis cross-classification now routes representation to a dedicated tokenizer contract, ELBO/random-future semantics to variational generation, and causal codec→generator→commit→SLO to the streaming contract; next gate is controlled implementation comparison | P2 |
...
| `causal-streaming-generation.md` | 658 | 28 | 5 | 4 | Four-layer codec→generator→commit→SLO contract, four clocks, rolling-noise/commit visual, bounded-memory lifecycle, formal-status/artifact audit and proposed StreamFork-1 are explicit; no checkpoint/runtime run was executed, so performance remains author-reported | P2 |
...
| `foundation-models.md` | 358 | 42 | 3 | 4 | System view separates tokenizer budgets, generator objectives and deployment variants; multi-view/4D adds a camera-time/state acceptance row, while streaming and bitstream claims retain separate evidence contracts | P2 |
...
| `evaluation.md` | 869 | 62 | 3 | 4 | Rewritten with 2025–26 task families, evaluator stress tests, pairwise statistics, SLO/energy, watermark and C2PA 2.4; restoration, causal/streaming and camera-time 4D now have distinct acceptance contracts | P2 |
...
| File | Lines | Refs | Visuals | Current depth | Main gap | Priority |
|---|---:|---:|---:|---:|---|---|
| `getting-started.md` | 277 | link-only | 1 | N/A | Navigation/tutorial page now blocks the camera-path→4D misconception and routes readers to the dedicated chapter; it is not scored as a subfield review | P2 |
...
| `timeline.md` | 887 | 0 local anchors | 2 Mermaid + 75 images | 4 | Tokenizer, causal/streaming and multi-view/4D lineages expose first-public/formal status and orthogonal contract layers separately; preprints remain labelled | P2 |
| `reading-list.md` | 533 | current external links | 2 | 4 | Tokenizer remains a shared prerequisite; the course now has seven branches, adding a dedicated multi-view/4D route with camera-time falsifiers and GridFork-1 | P2 |
...
| `bibliography.md` / registry | 162 / registry | 109 registered core items | 0 | N/A | Bibliography infrastructure is validated by identity/code/metadata integrity rather than the subfield visual checklist; next gate is deliberate chapter-local expansion rather than indiscriminate ingestion | P2 |
...
- **Backbone/scaling (next structural gap):** factorized/full spatiotemporal attention, MMDiT/expert normalization, 3D RoPE, noise-axis MoE, context parallelism, linear/hybrid/sparse attention and inter-step reuse; begin with official formal pages for LinVideo, TimeRipples/RAPID and current official artifacts for SANA-Video 2.0/Wan2.2, and do not infer a backbone mechanism from a product name.
