...
| `long-video-generation.md` | 405 | 29 | 1 | 4 | New 2026-09-02 review separates fixed-long, length extrapolation and open-horizon; synthesizes five combinable method routes, 2022–2026 primary-source milestones, five temporal evidence gates and proposed LongHorizon-1; no checkpoint/runtime run was executed | P2 |
...
6. ~~Mark the digital-human overview's `Identity 0.92` and `Temporal Consistency 0.90` as illustrative or regenerate it without fabricated-looking scores.~~ Corrected on 2026-08-30 by keeping the legacy file uncited and using a score-free contract visual; on 2026-08-31 the original path was additionally redrawn in Chinese, the numeric scores were removed, and the remaining evaluation bars were explicitly marked as illustrative rather than experimental results.
