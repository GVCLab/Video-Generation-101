...
1. taxonomy.md 不再嵌入静态任务地图或 078/079 位图；两张内联 Mermaid 是当前可维护源图，位图只保留为历史工件；
2. 导航剖面改为 $\mathcal N=(C,R,\Pi)$，其中 $\Pi=(F,\Gamma,H)$ 分别表示反馈、提交语义和评测跨度；
3. 正式任务合同改为 $\mathcal T=(I,O,K,\Delta,\Pi,E)$，导航维度不再冒充完整定义；
4. 补入[视频虚拟试衣专章](../docs/tasks/video-virtual-try-on.md)，并把 source-video VVT 与 pose-driven try-on animation 分开；
5. 将加权总分替换为不可补偿的硬门合取，并补齐联合音视频、多视角/4D、虚拟试衣等主张的专属证据。
...
4. 什么技术节点改变了任务合同，什么工作只推进了可复用技术底座或证据接口？
5. 每类任务主张对应的最小证据和反事实测试是什么？
