...
说明如何核对开放模型的发布内容、运行条件与可复现配置。

**前置知识：** 视频生成、基本软件环境管理。

**使用步骤：** 按输入输出和资源约束筛选模型 → 固定权重、代码、环境和采样参数 → 从小样本运行检查扩展到正式评测。
...
![开放视频模型从官方身份、版本冻结、发布内容拆分、许可交集、硬件预算、最小运行、质量复核到可复现清单的证据链；缺少权重、许可不相容或最小运行失败时进入隔离，而不是继续宣称可复现。](../assets/diagrams/open-video-model-release-surface.png)

**图 1：开放模型不是一个下载按钮。** 主链把模型身份、版本、发布物、许可证、硬件、运行证据和最终 manifest 连起来；红色出口表示证据不足时的隔离路径。上图是教学总览，下方 流程图及文字说明 是补充图示与文字说明。

![图 086：开放视频模型的可复现发布内容证据链](../assets/imagegen-diagrams/086/diagram.png)
顺序化文字替代：确认作者身份；固定代码和权重 revision；分别核对九个发布内容；求许可证交集；检查显存、磁盘、CUDA 和精度；用固定输入与随机种子跑最小样例；验证输出媒体契约；人工回读条件遵循和失败模式；最后保存 manifest、hash、命令、硬件和日志。任一关键门失败都应记录并隔离。
...
单个“最好看”的视频只能证明一次成功，不能证明模型稳定。最低限度应使用多个 prompt 与 seed，并统一输出规格：容器、编码、宽高、FPS、帧数、音轨和时长。FFmpeg 可检查媒体规格，但不能替代语义和物理评测 [[30]](#ref-30)。
...
<a id="ref-13"></a>[13] HPC-AI Tech. [Open-Sora official repository](https://github.com/hpcaitech/Open-Sora); [Open-Sora 2.0: Training a Commercial-Level Video Generation Model in $200k](https://arxiv.org/abs/2503.09642). Accessed 2026-08-30.
